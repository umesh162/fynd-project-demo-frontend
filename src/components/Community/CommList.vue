<template>
  <div>
    ...
    >
      <div class="px-1 pb-1 pt-2 hor-div">
        <div v-for="item in userJoinComm" :key="item._id" class="">
          <div
            class="flex items-center m-2 border-2 p-1 rounded-lg shadow-lg"
            :class="
              singleChannel.data._id === item._id
                ? 'text-white bg-gradient-to-r from-yellow-500 to-slate-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-cyan-300 dark:focus:ring-cyan-800'
                : 'bg-white'
            "
            v-on:click="singleChannelCall(item._id)"
          >
            <div class="w-16 hor-div-img">
              <img
      ...
      <div class="height overflow-auto scroll-hidden hide-all">
        <div class="px-2">
          ...
          <div
            class="flex items-center w-full my-3 shadow-lg rounded-lg border p-3"
            v-for="index in srcProd()"
            :key="index._id"
          >
            <div class="w-24 px-2" v-if="!!index.communityImage">
              <img
...
export default {
  ...
  computed: {
    ...mapState("comm", ["userJoinComm", "commList", "singleChannel"]),
    ...mapState("auth", ["userDetails"]),
  },
  methods: {
    async singleChannelCall(id) {
